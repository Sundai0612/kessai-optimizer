...
変更を提案すると、GitHub が自動で次の3つを確認します。すべて緑色のチェックが付けば問題ありません。

1. **書き方のチェック** — 書き間違いや使っていない記述がないか
2. **テスト** — `src/core/` の計算が正しい答えを出すか
3. **組み立ての確認** — 公開用に書き出せるか（型の食い違いもここで分かります）

赤い×が付いた場合は、その内容を見て直す必要があります。
